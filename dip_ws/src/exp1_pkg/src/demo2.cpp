...
void EqualizeHist(Mat src,  vector<int> nums){
    Mat dst = src.clone();
    int grey_sum = src.rows * src.cols;
    //灰度频率
    double grey_prob[256];
    //累计密度
    double grey_dis[256];
    //均值化后的灰度值
    int grey_equal[256];
    //均值化后的灰度值分布
    vector<int> dst_nums(256);
    //统计灰度频率
    for (int i = 0; i < 256; i++) {
        grey_prob[i] = ((double) nums[i] / grey_sum);
    }
    //计算累计密度
    grey_dis[0] = grey_prob[0];
    for (int i = 1; i < 256; i++){
        grey_dis[i] = grey_dis[i-1] +grey_prob[i];
    }

    //重新计算均衡化后的灰度值，四舍五入。参考公式：(N-1)*T+0.5
    for (int i = 0; i < 256; i++){
        grey_equal[i] = (uchar)(255 * grey_dis[i] + 0.5);
    }
